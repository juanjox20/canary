local activedExpBonus = 1 -- false is desatived
...
function talkaction.onSay(player, words, param)
	...
	elseif table.contains({"off", "no", "disable"}, split[1]) then
		...
		if activedExpBonus and activedExpBonus > 0 then
			player:sendTextMessage(MESSAGE_LOOK, "Experience bonus deactivated: -15%")
			player:setStorageValue(ServerStorage.Cast, 0)
		end
	...
	elseif table.contains({"on", "yes", "enable"}, split[1]) then
		...
		db.query("UPDATE `players` SET `cast_description` = '' WHERE `id` = " .. player:getGuid())
		if activedExpBonus and activedExpBonus > 0 then
			player:sendTextMessage(MESSAGE_LOOK, "Experience bonus actived: +15%")
			player:setStorageValue(ServerStorage.Cast, 1)
		end
	...
	elseif table.contains({"password", "guard"}, split[1]) then
		if split[2] then
			if table.contains({"off", "no", "disable"}, split[2]) then
				...
				if activedExpBonus and activedExpBonus > 0 then
					player:sendTextMessage(MESSAGE_STATUS, "Your experience bonus of 15% was reactivated.")
					player:setStorageValue(ServerStorage.Cast, 1)
				end
			else
				...
				if activedExpBonus and activedExpBonus > 0 then
					player:sendTextMessage(MESSAGE_STATUS, "Attention, Your experience bonus of 15% was deactivated.")
					player:setStorageValue(ServerStorage.Cast, 0)
				end
