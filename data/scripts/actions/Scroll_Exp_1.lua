...
function expscroll.onUse(cid, item, fromPosition, itemEx, toPosition)
    ...
    player:setStoreXpBoost(player:getStoreXpBoost() + 10) -- Suma el nuevo porcentaje al porcentaje de bonificación actual
    player:setExpBoostStamina(newBoost) -- Establecer el tiempo de bonificación de experiencia para representar 365 días
    
    Item(item.uid):remove(1)
    player:say('Your bonus of 10% XP has started and will last for the entire year!', TALKTYPE_MONSTER_SAY)
    return true
end
